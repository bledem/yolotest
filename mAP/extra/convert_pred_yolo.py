...
for tmp_file in txt_list:
  ...
  for fname in os.listdir('/home/ubuntu/sdcard/YOLOv2-master/XmlToTxt/water_bottle_img'):
    if fname.startswith(image_name):
      ## image found
      print(fname)
      img = cv2.imread('/home/ubuntu/sdcard/YOLOv2-master/XmlToTxt/water_bottle_img/' + fname)
      ...
      with open(tmp_file, "a") as new_f:
        for line in content:
          ...
          new_f.write('%s %s %s %s %s %s\n' % (obj_name, str(conf), str(left), str(top), str(right),str(bottom)))
