...
print("first class 0 is ", obj_list[0])
...
if len(txt_list) == 0:
    ...
    sys.exit()
print("loop to be done", len(txt_list))
for i, tmp_file in enumerate(txt_list):
  ...
  #path = os.path.abspath(os.path.join("/home/ubuntu/YOLOv2-master/XmlToTxt/water_bottle_img", )
    for fname in os.listdir('/home/ubuntu/YOLOv2-master/XmlToTxt/water_bottle_img'):
        if fname.startswith(image_name):
          ## image found
          #print(fname)
          img = cv2.imread('/home/ubuntu/YOLOv2-master/XmlToTxt/water_bottle_img/' + fname)
       ...
       bbox_alone.append(tmp_file)
       os.rename(tmp_file, "not_used/" + tmp_file)

        #sys.exit()
print("this one have been excluded", bbox_alone)
