...
class CoordsToBoxesYOLO(object):
    ...
    def __call__(self, img, cds):
        ...
        h, w = img.shape[:2]
        #print(h,w, "in YOLO convertor")
        boxcds = np.split(np.array(cds), np.arange(1, num) * 4)
        boxes = np.array(
            [self.coords2yolobbox(_, w, h) for _ in boxcds], dtype=float)
        boxes_yolo = np.array([self.convert((w,h), _) for _ in boxes], dtype=float)
...
###show boolean to show the initial and augmented resulting image
def data_augmentation(inputs, transformDA, observer=count, train=True, show=True):
    new_batch=[]
    da = np.random.uniform(0, 1)>0.85
    if (da):
        print("da batch")
        for elt in inputs:
            global count

            #print([len(elt[i]) for i in range(len(elt))])
            img, label= elt
            #print("img shape in dataTrans", img.shape)
            in_shape= img.shape
             #boxes shaped into the format [[x_min,y_min,x_max,y_max]]
            yolo_boxes = [ Box(elt[0]*in_shape[2], elt[1]*in_shape[1], elt[2]*in_shape[2],elt[3]*in_shape[1]) #to check
                     for elt in label.copy()]
            boxes = [[bb.int_left_top()[0], bb.int_left_top()[1], bb.int_right_bottom()[0], bb.int_right_bottom()[1]]
                     for bb in yolo_boxes]
            tag_label = label[:,4:] #to check


           # print("yolo_boxes", len(yolo_boxes), boxes)
            im = np.transpose(img.copy(), (1, 2, 0))

            #random_angle = np.random.uniform(0, 360)
            im_c = imcv2_recolor(im)
            im = np.asarray(im, dtype=np.float32) /255.0
            im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)

            #im, trans_param = imcv2_affine_trans(im)
           # print( "shape init", in_shape, "shape for the funcion" , im.shape) -> shape init (3, 448, 448) shape for the function (448, 448, 3)
            #transformation
            transformed_image, t_boxes = transformDA(im_c, boxes)
            #print("boxes from in_label", label, label.shape, "tag", tag_label, "t_boxes", t_boxes, t_boxes.shape)

            result = np.array(np.concatenate((t_boxes, tag_label), axis=1))
           # print("result", result, result.shape)

            #print("transformed_image", transformed_image.shape)

            if show==True:
                yolo_show = [ Box(elt[0]*in_shape[2], elt[1]*in_shape[1], elt[2]*in_shape[2],elt[3]*in_shape[1]) #to check
                         for elt in t_boxes.copy()]
                boxes_show = [[bb.int_left_top()[0],bb.int_left_top()[1],  bb.int_right_bottom()[0] , bb.int_right_bottom()[1]]
                                  for bb in yolo_show]
                #print("boxes from in_label", label, "t_label ", result, "yolo_show 8x,y,w,h)", yolo_show, "after YOLO computation", boxes_show)
                show_image = transformed_image.copy()
                cv2.rectangle(
                           show_image,
                           (int(boxes_show[0][0]), int(boxes_show[0][1])), (int(boxes_show[0][2]), int(boxes_show[0][3])),
                           (255, 0, 255),3)
                cv2.rectangle(
                           im,
                           (int(boxes[0][0]), int(boxes[0][1])), (int(boxes[0][2]), int(boxes[0][3])),
                           (255, 0, 255),3)
                show_image_resized = cv2.resize(show_image.copy(), (in_shape[1], in_shape[2]))
                cv2.imshow("image", show_image)
                cv2.imshow("imageInit", im)
                cv2.waitKey(0)
                cv2.destroyAllWindows()

            transformed_image = np.asarray(np.transpose(transformed_image, (2, 0, 1)), dtype=np.float32)

            count+=1
        #    reporter.add_observer('data_aug', observer)
        #    observation={}
        #    with reporter.scope(observation):
        #        reporter.report({'count': count}, observer)
            #print("augmented data observation", count)

            if count%freq==0:
                print("augmented data observation", count)
            new_batch.append([transformed_image, label])
        return new_batch, count
